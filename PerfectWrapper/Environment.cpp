...
"PathWaypoint", "Random", "PluginDrag", "ElapsedTime", "Delay", "delay", "spawn", "Spawn"
		...
		bool CreateEnvironment()
		{
			...
			if (GL)
			{
				...
				if (Run(unprotect))
				{
...
#endif
					TaskScheduler::UpdateTaskScheduler();
#if CorePrints
					...
					lua_setglobal(GL, "KRONOS_LOADED", CustomFunctions::KRONOS_LOADED);
		...
		int Decider(lua_State* L)
		{
			...
			if (RobloxFunc::CheckRobloxObject(L, 1))
			{
		...
		int RobloxAttached(lua_State* L)
		{
			...
			FuncWrap(Thread, "", "ipairs");
			FuncWrap(Thread, "", "pairs");
	...
	namespace Wrapping
	{
		...
		void Wrap(lua_State* L, uintptr_t RL, const int32_t idx)
		{
			...
			{
				r_lua_rawgeti(RL, LUA_REGISTRYINDEX, reinterpret_cast<uintptr_t>(lua_topointer(L, idx)));
		...
		void Wrap(uintptr_t RL, lua_State* L, const int32_t idx)
		{
			...
			else if (Type == R_LUA_TLIGHTUSERDATA)
			{
				...
				const uintptr_t Ref = reinterpret_cast<uintptr_t>(lua_topointer(L, -1));
			...
			else if (Type == R_LUA_TUSERDATA)
			{
				...
				if (lua_isnil(L, -1))
				{
					...
					void* NUD = reinterpret_cast<void*>(lua_newuserdata(L, 0));
	...
	namespace YieldingHandling
	{
		...
		void PcallOnSuccess(lua_State* L)
		{
			...
			{
				if (Dom != Space->Main && Space->Coroutine != true)
				{
...
*/
		...
		*/
		int VanillaCall(uintptr_t RL)
		{
			...
			printf("%s Called\n", __FUNCTION__);
#endif
			uint32_t Ref = r_lua_tonumber(RL, lua_upvalueindex(1), NULL);
			lua_State* Prior = reinterpret_cast<lua_State*>(r_lua_touserdata(RL, lua_upvalueindex(2)));
			if (Ref == 0)
			{
				uintptr_t CI = *reinterpret_cast<uintptr_t*>(RL + 24);
				uintptr_t Env = *reinterpret_cast<uintptr_t*>(**reinterpret_cast<uintptr_t**>(CI + 8) + 16);
	...
	namespace CustomFunctions
	{
		...
		int GIndex(lua_State* L)
		{
			...
			printf("%s Called\n", __FUNCTION__);
#endif
			lua_getglobal(GL, lua_tostring(L, 2));
			lua_xmove(GL, L, 1);
			return 1;
		}
...
#endif
			lua_xmove(L, GL, 1);
			lua_setglobal(GL, lua_tostring(L, -1));
			return 0;
		}
		...
		int ReadFile(lua_State* L)
		{
			...
			lua_pushstring(L, Tools::Files::ReadFile(Tools::Files::GetDLLPath("\\workspace").append("\\" + std::string(Name))).c_str());
			return 1;
		}
		...
		int GetConnections(lua_State* L)
		{
			...
			*/
			uintptr_t RL = ExtraSpace::GetRoot(L)->RL;
			Wrapping::Wrap(L, RL, 1);
			const uintptr_t Event = *reinterpret_cast<uintptr_t*>(r_lua_touserdata(RL, -1));
			const auto Source = reinterpret_cast<std::weak_ptr<uintptr_t>*>(Event + 4)->lock();//+0x4 In Struct
			if (Source != nullptr)
			{
				const uintptr_t GetOrCreate = *reinterpret_cast<uintptr_t*>(*reinterpret_cast<uintptr_t*>(Event));//EventDesc VFTable
				printf("GetOrCreate: 0x%X\n", noaslr(GetOrCreate));
				printf("Event: 0x%X\n", Event);
				printf("Source: 0x%X\n", Source.get());
				printf("Connection Name: %s\n", reinterpret_cast<const char*>(Source.get()));
				//const uintptr_t* Signal =  reinterpret_cast<uintptr_t*(__thiscall*)(uintptr_t*, bool)>(GetOrCreate)(Source.get(), true);
				lua_newtable(L);
			}
			else
			{
	...
	namespace RobloxFunc
	{
		...
		bool r2_lua_getupvalue(uintptr_t RL, int32_t Clo, int32_t Index)
		{
			...
			{
				int UpValue = clo + 16 * (Index + 1);//Need to improve this after source is usable
